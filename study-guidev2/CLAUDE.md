...
Run from this directory (`study-guidev2/`):
...
- `./deploy.sh ["commit msg"]` — builds, then syncs `dist/` into the sibling `../study-guides-deploy/` repo (`DEPLOY_DIR` override supported) and pushes to GitHub Pages (`https://mushcore.github.io/study-guides/`). Assumes `../study-guides-deploy/.git` exists.
