...
**Forgetting the `async` keyword is the mirror mistake.** Writing `await` inside a function declared without `async` is a `SyntaxError`. The function declaration must include `async` — there is no implicit upgrade based on the presence of `await`.
