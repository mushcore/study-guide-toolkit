...
> **Pitfall:** Omitting `return` does not log the last computed value — it logs `undefined`. `console.log(foo(3))` where `foo` has no `return` prints `undefined`, not `3`. ISAQuiz3 tests this directly.
...
> **Pitfall:** `checkString("John")` returns `true` but prints **nothing**. The return value is discarded when no `console.log` wraps the call. ISAQuiz3 tests this pattern.
