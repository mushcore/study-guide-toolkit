...
**Pitfall:** Adding a single feature to a monolithic app means shutting down the entire application. MSA avoids this by letting you redeploy individual services independently.
...
**Pitfall:** MSA and SOA refer to the same thing in this course. Do not contrast them or treat SOA as a separate, older alternative.
...
**Example:** Your term-one apps with no server-side scripting were effectively serverless — you sent API requests to image or data servers that someone else operated, and received processed results in return.
