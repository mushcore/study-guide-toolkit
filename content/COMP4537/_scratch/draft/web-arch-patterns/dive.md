...
**Pitfall:** MSA and SOA are treated as equivalent in this course. The slide notes "MSA: Microservice Architecture / SOA: Service-Oriented Architecture — slightly different but refer the same thing in this course." Do not contrast them or treat SOA as a predecessor architecture.

**Pitfall:** Monolithic architecture is not inherently "bad." It is the appropriate choice for prototyping, MVPs, and apps that do not expect frequent independent feature updates. The costs (full-shutdown deploys, no independent scaling) only matter when complexity grows.
