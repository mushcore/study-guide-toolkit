...
related: [promises-intro, promise-chaining, error-handling]
...
> **Pitfall:** `await` is only valid inside an `async` function. Using it in a regular function causes a `SyntaxError`. Mark the enclosing function `async` before adding any `await`.
