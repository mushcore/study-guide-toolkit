...
**Pitfall:** GET has a URL size limit; POST does not. The claim "there are no size limits for both GET and POST" is false. GET data goes into the URL query string, which browsers and servers cap at a few kilobytes. POST data goes in the request body — no size limit. Exam questions present this as a true/false trap.

**Pitfall:** PUT vs PATCH confusion — PUT is idempotent, PATCH is not. PUT replaces the entire resource at a URI with the request payload. Sending PUT with a partial object overwrites the rest of the fields with nothing. PATCH applies only the fields you send. Confusing them in an exam scenario (e.g., "edit one profile field") costs marks — the answer is PATCH, not PUT.
