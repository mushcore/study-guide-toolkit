...
In 2013, Adobe lost 153 million user records to a breach. Passwords were encrypted — not hashed. Attackers decrypted them using a stolen key. Every user was compromised in one step. The correct defense was hashing: one-way, irreversible, no key to steal.
...
> **Pitfall**
> Encrypting passwords instead of hashing is a security misconfiguration. An attacker with server access — through a vulnerability, insider threat, or subpoena — gains the decryption key and can reverse every stored password. Hashing removes that attack surface entirely.
