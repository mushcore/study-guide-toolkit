...
A server handling thousands of requests per day cannot afford a database lookup on every one just to confirm who the requester is. JSON Web Tokens solve this: the token itself carries proof of identity, and the server verifies it cryptographically — no session store required.
...
> **Pitfall**
> The three parts of a JWT are header, payload, and signature — NOT header, payload, and certificate. NOT header, body, and footer. ISAQuiz11 Q5 distractors use both wrong terms. Certificate and Body are not JWT concepts.
...
> **Pitfall**
> Omitting both `HttpOnly` and `Secure` on the JWT cookie exposes the token to XSS attacks, CSRF attacks, and unauthorized access — all three, not just XSS. ISAQuiz11 Q8 requires selecting "All of the above." Selecting only "XSS" scores zero.
