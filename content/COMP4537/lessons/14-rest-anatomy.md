...
> **Pitfall**
> Sending credentials via GET is not safe. GET data appears in the URL — the username and password are visible in browser history, server logs, and any proxy between client and server. Always use POST (or a more secure mechanism) when sending credentials.

> **Pitfall**
> "There are no size limits for both GET and POST" is false. GET appends data to the URL; URLs have a length limit enforced by browsers and servers. POST places data in the body and has no size limit. Exam questions test this directly.
