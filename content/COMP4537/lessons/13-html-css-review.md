...
> **Pitfall**
> `color` and `background-color` are different properties. Writing `color: blue` on a `<div>` turns the **text** blue, not the background. To set the background, use `background-color: blue`. ISAQuiz1 tests this distinction directly.

> **Pitfall**
> A comma in a selector groups elements: `h1, p { }` targets both `<h1>` and `<p>`. Without the comma, `h1 p { }` selects a `<p>` descendant of `<h1>` — a completely different rule.
