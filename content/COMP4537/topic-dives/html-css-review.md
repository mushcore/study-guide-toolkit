...
> **Takeaway** The CSS box model stacks content → padding → border → margin from inside out. Commas group independent selectors; spaces create descendant relationships — a critical distinction in selector questions. `<!DOCTYPE html>` precedes `<html>` and is a declaration, not an element.
