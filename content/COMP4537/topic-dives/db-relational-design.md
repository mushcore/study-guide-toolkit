...
> **Example**
> Given two tables — `person(personID PK, name, dob)` and `birthCertificate(certID PK, personID FK, issueDate)` — identify the relationship type. (Source: ISAQuiz10 Q4)
>
> **Step 1 — Locate the foreign key.** `birthCertificate.personID` references `person.personID`.
>
> **Step 2 — Ask: can the FK column repeat?** If multiple certificate rows can share the same `personID`, the relationship is 1:M. If the FK column is UNIQUE — each `personID` can appear at most once — it is 1:1.
>
> **Step 3 — Conclude.** The constraint says each person has exactly one birth certificate; each certificate belongs to exactly one person. `personID` in `birthCertificate` is unique per row. The relationship is **1:1**.

**Rule:** Check the cardinality of the foreign key column. Repeatable → 1:M. Constrained UNIQUE → 1:1.
...
> **Pitfall**
> A table can satisfy 1NF and still violate 2NF. Consider a table with `PersonID (PK)`, `Name`, `City`, `Country`. Each cell is atomic — 1NF passes. But `Country` is determined by `City`, not by `PersonID`. That partial dependency violates 2NF. Quiz 10 Q5 correct answer: "violates 2NF, but complies with 1NF."
...
> **Pitfall**
> `SELECT * FROM Tabel1, Tabel2` — a Cartesian join with no WHERE clause — always returns exactly n×m rows. Identical column names in both tables do **not** reduce the row count. Column name collisions affect which column values appear in the result set, not how many rows are returned. (ISAQuiz10 Q6)
...
> **Takeaway**
> Three rules cover all relational design decisions. First: one table per entity. Second: link entities with primary key / foreign key pairs — the parent table must exist first. Third: check normal forms — 1NF requires atomic values; 2NF requires every non-key column to depend on the whole primary key, not on another non-key column. A Cartesian join without WHERE always returns n×m rows regardless of column name overlap.
