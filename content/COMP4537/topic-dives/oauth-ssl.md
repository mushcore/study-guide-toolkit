...
sequenceDiagram
    ...
    Note over C,G: Step 0 (one-time): app owner registers → client_id + client_secret
    U->>C: Click "Login with Google"
    C->>G: Redirect: client_id + scope
    G->>U: Consent screen (what data will be shared)
    U->>G: User clicks Allow
    G->>C: Bearer token issued
...
> **Pitfall**
> The client secret and the bearer token are different credentials. The client secret identifies the app (stays server-side permanently). The bearer token authorizes access to one user's data (time-limited, issued per login). Confusing them or exposing either to the browser is a security misconfiguration. Storing the client secret in frontend JavaScript exposes it to anyone who inspects the page source.
