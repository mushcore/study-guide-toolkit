...
> **Pitfall**
> Callback nesting beyond two levels produces "callback hell" — error handling duplicates at every level, indentation grows horizontally, and control flow is hard to follow. A single `.catch()` on a promise chain replaces all those scattered `if (err) return` checks.
...
> **Example**
> Trace the SELECT → UPDATE promise chain step by step.
...
> **Takeaway**
> Any two async operations whose second depends on the first's result must be linked by a returned Promise inside `.then()` or by `await`. Omitting this creates a silent race condition — no error is thrown, but the UPDATE may execute before the SELECT result arrives. `con.promise()` converts the mysql callback API to Promise-based; destructure `[rows]` from the result to discard the fields metadata.
