...
> **Pitfall** `var` hoists the **declaration** only. Reading `x` before `var x = 42` returns `undefined`, not a `ReferenceError`. Expecting an error here is the most common exam mistake.
>
> ```js
...
> **Pitfall** `let` and `const` outside their block throw `ReferenceError`, not `undefined`. Do not apply `var`-hoisting logic to `let`/`const`.
>
> ```js
> if (true) { let a = 5; }
...
> **Pitfall** Any function that exits without `return <value>` returns `undefined`. The internal computation is discarded — it never reaches the caller.
>
> ```js
...
> **Takeaway** `var` hoists its declaration to function scope; `let`/`const` stay block-scoped and throw `ReferenceError` before their line. Functions without `return` silently return `undefined`. The event loop drains all microtasks before each macrotask — Promise `.then()` always runs before `setTimeout`, even with a 0 ms delay.
