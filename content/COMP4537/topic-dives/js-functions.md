...
> **Pitfall**
> Using "parameter" and "argument" interchangeably is wrong. Every quiz that covers this topic tests the distinction by name. Commit both terms as separate definitions, not synonyms.
...
> **Pitfall**
> Forgetting `return` is a logic error JavaScript does not flag. The function runs, produces an internal result, then discards it. `console.log(foo(3))` outputs `undefined`, not `3`.
...
> **Takeaway** Parameters are definition-site names; arguments are call-site values. A function without `return` returns `undefined` — the computed value is invisible to the caller. Closures keep their enclosing scope alive: variables from the outer function survive as long as the inner function exists.
