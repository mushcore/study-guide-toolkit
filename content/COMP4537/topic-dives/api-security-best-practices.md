...
> **Pitfall**
> `.replace()` is not a SQL injection fix. It strips specific characters but is bypassable with other encodings. Parameterized queries with `?` placeholders are the only reliable prevention. ISAQuiz12 Q2 and exam-strategy Pitfall 5 both confirm this.
