...
> **Example**
> Trace the three-step Node.js MySQL connection pattern from require to query result.
...
> **Takeaway**
> The three-step pattern (install → createConnection → connect+query-inside-callback) is identical locally and on cPanel — only the `host` value changes. Keep all connection code server-side: credentials in client-side JS are visible to every visitor via DevTools. `mysql` is external — always `npm install mysql` before `require('mysql')`.
