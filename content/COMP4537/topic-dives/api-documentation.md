...
> **Pitfall**
> Writing a `swagger.json` from scratch almost always produces invalid structure on the first attempt. The `openapi`, `info`, and `paths` top-level keys must be present and correctly formatted before any tooling will parse the file. Start from the editor.swagger.io sample — it provides a structurally valid skeleton. Only replace the content inside `paths`; do not attempt to rebuild the outer skeleton.
