...
sequenceDiagram
    ...
    Server-->>Browser: Set-Cookie: token=abc; HttpOnly; Secure; SameSite=None
    Note over Browser: JS cannot read this cookie (document.cookie = "")
    Browser->>Server: GET /profile (Cookie auto-attached by browser)
    Server-->>Browser: 200 OK (profile data)
    Browser->>Server: POST /logout
    Server-->>Browser: Set-Cookie: token=; Max-Age=0; HttpOnly; Secure; SameSite=None
