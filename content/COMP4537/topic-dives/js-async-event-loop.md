...
> **Pitfall** The Promise executor is NOT deferred. It runs the instant `new Promise(executor)` is evaluated — before any following synchronous statement. Confusing eager execution with `setTimeout` deferral is the most common source of wrong output predictions on async ordering questions.
...
> **Takeaway** JavaScript is single-threaded non-blocking: I/O is delegated to the OS; callbacks return via the event queue. The event loop always drains the entire microtask queue (Promises) before pulling the next macrotask (`setTimeout`). Promise executors run eagerly — synchronously on construction, before any code that follows.
