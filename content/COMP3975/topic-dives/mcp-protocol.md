...
> **Example:** Full JSON-RPC 2.0 tool invocation cycle
>
> **Step 1 — Client discovers tools.**
> Client sends `{"jsonrpc":"2.0","method":"tools/list","id":1}`. Server replies with an array of tool objects (name, description, inputSchema).
>
> **Step 2 — Client selects a tool and calls it.**
> Client sends `{"jsonrpc":"2.0","method":"tools/call","params":{"name":"AddTodoTool","arguments":{"title":"Buy milk"}},"id":2}`.
>
> **Step 3 — Server executes and responds.**
> Server runs the tool's `handle()` method and returns `{"jsonrpc":"2.0","result":{"id":42,"title":"Buy milk"},"id":2}`.
>
> **Step 4 — Composability: this server is also a client.**
> If the AddTodoTool itself calls another MCP server (e.g. a notification server) using the same JSON-RPC envelope, the entire chain operates without custom protocol bridging.
