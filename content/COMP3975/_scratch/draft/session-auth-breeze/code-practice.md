...
3. **Using `auth()->user()` instead of `Auth::user()`.** Both resolve to the same object, but the professor uses the facade form `Auth::user()` throughout the course materials. Use facade style in exam answers.
