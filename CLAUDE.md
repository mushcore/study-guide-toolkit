...
- `study-guidev2/` — React + Vite single-page app that consumes the bundles. Dev / build / preview commands run from there.
- `COMP{4736,4870,4911,4915}/` — per-course source directories. `materials/` is ground truth; `generated/` and `graphify-out/` are secondary authoring sources (old diagnosis reports, topic research notes, knowledge-graph reports). Four course ids only.
- `study-guides-deploy/` — GitHub Pages deploy target for the v2 build. `./study-guidev2/deploy.sh` pushes to it.
...
| App architecture, dev commands, route shapes | `study-guidev2/CLAUDE.md` |
...
- `/add-course {id}` — Phase B automation: calls `/audit-content` preflight, compiles the bundle, wires the three hardcoded touchpoints (`scripts/build-content.js`, `content/_dist/_aggregator.js`, `study-guidev2/src/main.jsx`), handles annotation-variant dispatch in `App.jsx`, verifies `npm run build`.
