...
- INCLUDE synthesis sessions: in the final third of the study plan, schedule
  cross-pillar review using surprising connections from
  `graphify-out/GRAPH_REPORT.md` (if it exists). Use `/flashcards synthesis`
  to generate cross-topic questions bridging RUP and EJB. These test
  higher-order thinking (Apply/Analyze) and prep for essay/short-answer
  sections.

## Input
- Days until exam: $ARGUMENTS (or read from CLAUDE.md exam date)
- Available hours per day: from CLAUDE.md (1-2 hours/day)
