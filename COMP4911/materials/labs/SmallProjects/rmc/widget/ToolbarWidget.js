...
dojo.widget.defineWidget(
	...
	// properties and methods
	{
		...
		//workitem41170
		ID_toolbar_collapse_item : "toolbar_collapse_item",
		...
		//workitem41170
		text_toolbar_collapse_item : theApp.res.toolbar_tooltip_collapse,
		...
		attachTemplateNodes: function() {
			...
			//workitem41170
        	this._createMenuItem(theApp.res.toolbar_menu_collapseTreeItem, function(e) {
				self._collapseAllItem();
			});
		...
		onButtonClick: function(evt){
			...
			if (node.id == this.ID_toolbar_show_glossary) {
				...
				this._showInfoPage();
				//workitem41170
			} else if (node.id == this.ID_toolbar_collapse_item) {
				this._collapseAllItem();
		...
		_sendFeedback : function() {
			...
			if (feedbackUrl.indexOf("http") < 0) {
				...
				//by shijin Defect41739 "The value of len has been changed and not 225 any more."
				var whole_len = 225;			
				if (str.length > whole_len) {
					var nobody_len = feedbackUrl.length;
				 	if (nobody_len > 123) {
				 		if (feedbackUrl.substring(nobody_len-1) == "\"") {
				...
				//location.href=str; changed by shijin because of RATLC02515506(Defect 42023)
				var callEmailClient = document.createElement('iframe');
				callEmailClient.setAttribute('style', 'display:none;');
				callEmailClient.setAttribute('src',str);
				document.getElementsByTagName("BODY")[0].appendChild(callEmailClient);
			}
			else {
		...
		//workitem41170
		_collapseAllItem : function() {
			theApp.collapseAllTreeItem();
		},
